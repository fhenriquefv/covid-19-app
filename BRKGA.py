...
class BRKGA:
    ...
    def __init__(self, cp_array, ct_array, c_size, p_size, elite, mutant, n_it, obj=False):
        ...
        if(type(cp_array) is list or type(cp_array) is np.ndarray):
            ...
            raise ValueError(
                "The value of the parameter \"cp_array\" is invalid, please enter a valid list or ndarray.")

        if(type(ct_array) is list or type(ct_array) is np.ndarray):
            self._cost = ct_array
        else:
            raise ValueError(
                "The value of the parameter \"ct_array\" is invalid, please enter a valid list or ndarray.")

        if(type(p_size) is int or type(p_size) is np.int64):
            self._psize = p_size
        else:
            raise ValueError(
                "The value of the parameter \"p_size\" is invalid, please enter a valid integer.")

        if(type(c_size) is int or type(c_size) is np.int64):
            self._csize = c_size
        else:
            raise ValueError(
                "The value of the parameter \"c_size\" is invalid, please enter a valid integer.")

        if(type(elite) is int or type(c_size) is np.int64):
            self._elite = elite
        else:
            raise ValueError(
                "The value of the parameter \"elite\" is invalid, please enter a valid integer.")

        if(type(mutant) is int or type(c_size) is np.int64):
            self._mutant = mutant
        else:
            raise ValueError(
                "The value of the parameter \"mutant\" is invalid, please enter a valid integer.")

        if(type(n_it) is int or type(c_size) is np.int64):
            self._n = n_it
        else:
            raise ValueError(
                "The value of the parameter \"n_it\" is invalid, please enter a valid integer value.")

        if(type(obj) is bool or type(c_size) is np.int64):
            self._obj = obj
        else:
            raise ValueError(
                "The value of the parameter \"obj\", please enter a valid boolean.")
    ...
    def Crossover(self, parent1, parent2):
        ...
        for i in range(0, len(common_values)):
            tpl[common_values[i]] = (
                p1[p1 == common_values[i]].index[0], p2[p2 == common_values[i]].index[0])
        ...
        for i in range(0, len(p1)):
            ...
            if(coin <= 0.5):
                ...
                if p1[i] in common_values:
                    son[tpl[p1[i]][1]], son[tpl[p1[i]][0]
                                            ] = son[tpl[p1[i]][0]], son[tpl[p1[i]][1]]
                elif p2[i] in common_values:
                    son[tpl[p2[i]][1]], son[tpl[p2[i]][0]
                                            ] = son[tpl[p2[i]][0]], son[tpl[p2[i]][1]]
                else:
        ...
        while(count < self._n):
            ...
            if count == 0:
                ...
                for i in range(0, self._psize):
                    ...
                    obj, decoded_chromossome = self.Decoder(
                        chromossome, self._obj)
                    solutions = solutions.append(
                        pd.Series(obj), ignore_index=True)
                    population.append(decoded_chromossome)
                # creates the elites chromossomes
                elite_sol, elite = self.Elite(
                    solutions, population, self._elite)
                ...
                for i in range(0, self._elite):
                    ...
                    next_solutions = next_solutions.append(
                        pd.Series(elite_sol[elite_sol.index[i]]), ignore_index=True)
                ...
                for i in range(0, self._mutant):
                    ...
                    next_solutions = next_solutions.append(
                        pd.Series(mutant_sol[i]), ignore_index=True)
                ...
                for i in range(0, p_left):
                    ...
                    normal_parent = population[np.random.randint(
                        0, len(population))]
                    ...
                    next_solutions = next_solutions.append(
                        pd.Series(sol), ignore_index=True)
            else:
                population = []
                solutions = pd.Series()
                ...
                for i in next_solutions.index:
                    solutions = solutions.append(
                        pd.Series(next_solutions.loc[i]), ignore_index=True)
